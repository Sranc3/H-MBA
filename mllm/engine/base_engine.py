...
class Seq2Seq2DataCollatorWithImage(Seq2SeqDataCollator):
    ...
    def _video_process(self, features: List[Dict[str, Any]]) -> Dict[str, Any]:
        
        videos = [feature['video'] for feature in features]
        video_list = []
        for video in videos:
            vid = torch.stack(([img for img in video]), dim=0)
            video_list.append(vid)
        videos = torch.stack(video_list, dim=0)
        ret = dict(videos=videos)
        return ret
    ...
    def __call__(self, features: List[Dict[str, Any]], return_tensors=None) -> Dict[str, torch.Tensor]:
        ...
        video_outputs = self._video_process(features)
        ret.update(video_outputs)
