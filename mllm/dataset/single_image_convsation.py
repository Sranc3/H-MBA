...
class SingleImageConvDatasetMixin:
    ...
    def __getitem__(self, index, debug_mode=False, return_conv=False) -> Dict[str, Any]:
        ...
        video: List[Image.Image] = item.get('video', None)
        ...
        if isinstance(image, list):
            ...
            if self.transforms is not None and image is not None:
                ...
                if video is not None:
                    transformed_video=[]
                    for ele in video:
                        ele, tgt = self.transforms(ele, None)
                        transformed_video.append(ele)
    ...
    def process_video(self, video: List[Image.Image]) -> Dict[str, Any]:
        video_list = []
        for img in video:
            video_list.append(self.process_func['image'](img, self.preprocessor)['image'])
        video_dict = {'video':video_list}
        return video_dict
