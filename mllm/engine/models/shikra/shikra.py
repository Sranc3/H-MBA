...
from mamba_adaptation import H_MBA

import torchvision
from matplotlib import pyplot as plt
def vis_tensor(tensor):
    ori_img = tensor.reshape(1,16,16,-1).squeeze()
    img =  ori_img.cpu()
    img = img.permute(2,0,1).mean(0)
    plt.imshow(img)
    plt.savefig('kb_feature.jpg', dpi=300)
...
class ShikraLlamaModel(LlamaModel):
    ...
    def __init__(self, config: LlamaConfig, mm_vision_tower=None, mm_hidden_size=None):
        ...
        if hasattr(config, "use_mamba"):                                                                                  
            ...
            # self.mamba_reverse = Mamba(d_model=1024, d_state=16, d_conv=4, expand=2)
            self.hmba = H_MBA(d_model=1024, n_layer=1, d_state=16, d_conv=4, expand=2)
            ...
            with torch.no_grad():
               ...
               ###################################################################################
                ...
                for i in range(batch):
                    ...
                    video_features=[]
                    for img in video:
                        image_forward_out = vision_tower(img.unsqueeze(0), output_hidden_states=True)
                        select_hidden_state_layer = getattr(self.config, "mm_vision_select_layer", -1)
                        select_hidden_state = image_forward_out.hidden_states[select_hidden_state_layer]
                        img_feature = select_hidden_state[:, 1:]
                        video_features.append(img_feature)
                    video_features = torch.stack([img_feature for img_feature in video_features], dim=0).squeeze()
                    videos_list.append(video_features)
                        
            videos_features = torch.stack(videos_list,dim=0)
            whole_video = torch.cat((videos_features,image_features.unsqueeze(1)),dim=1)
            mamba_out = self.hmba(whole_video,image_features)
            image_features = mamba_out
